...
import { createPost, decrementPostLikes, deletePost, getAllComments, get_all_posts, increasePostLikes, postComment } from "@/config/redux/action/postAction";
      ...
      <UserLayout>
        <DashBoardLayout>
          <div className={styles.scrollComponent}>
            ...
            <div className={styles.createPostContainer}>
              <img className={styles.userProfile} src={`${authState.user?.userId?.profilePictureUrl}`} alt="" />
                ...
                {postState.posts.map((post)=>(
                    <div key={post._id} className={styles.singleCard}>
                      <div className={styles.singleCard_profileContainer}>
                        <img src={`${post.userId.profilePictureUrl}`} alt="" />
                        <div>
                          <div style={{display:"flex",gap:"1.2rem",justifyContent:"space-between"}}>
                            ...
                            {post.userId._id === authState.user.userId._id && <div onClick={async ()=>{
                              ...
                              await dispatch(get_all_posts());
                              }} style={{cursor:"pointer"}}><svg style={{height:"1.4em",color:"red"}} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                          ...
                          <div className={styles.optionContainer}>
                            <div className={styles.singleOption_optionContainer}>
                              <svg onClick={async ()=>{
                                // if(!haslikedPost){
                                //   await dispatch(increasePostLikes({post_id:post._id}));
                                // }else{
                                //   await dispatch(decrementPostLikes({post_id:post._id}));
                                // }
...

        {
          ...
          <div onClick={()=>dispatch(resetPostId())} className={styles.commentsContainer}>
            <div onClick={(e)=>e.stopPropagation()/*this is to stop event to propagate to its parent */} className={styles.allCommentsContainer}>
                  ...
                  {postState.comments.map((comment)=>(
                    <div className={styles.singleComment} key={comment._id}>
                      <div className={styles.singleComment_profileContainer}>
                        <img src={comment.userId.profilePictureUrl} alt="#" />
                        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                          <p style={{fontWeight:"bold",fontSize:"1.2rem"}}>{comment.userId.name}</p>
                          <p>@{comment.userId.username}</p>
                        </div>
                      </div>
                      <p style={{background:"rgba(179,179,179,1)",padding:"1rem"}}>{comment.body}</p>
                    </div>
                  ))}
                </div>
              }
              <div className={styles.postCommentContainer}>
                ...
                <div onClick={async ()=>{
                  ...
                  setCommentText("");
                }} className={styles.postCommentButton} >Comment</div>
              </div>
            </div>
          </div>
        }
